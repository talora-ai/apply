    ...
    def analyze(self, content: bytes) -> AtsDiagnosticData:
        ...
        try:
            ...
            score = self._score(
                ...
                high_visual_density=high_visual_density,
            )
    ...
    def _score(
        *,
        ...
        if multi_column_pages:
            ratio = multi_column_pages / max(pages, 1)
            score -= round(35 * min(ratio, 1.0))
        if high_visual_density:
            score -= 12
        if images_count:
            score -= min(8, images_count * 2)
        if drawings_count > 20 * pages:
            score -= 5
        if average_characters < 300:
            score -= 15
        elif average_characters < 600:
            score -= 5

        return max(0, min(100, score))

    @staticmethod
    def _has_multiple_columns(blocks: list[Any], words: list[Any], page_width: float) -> bool:
        ...
        for line_words in lines.values():
            ...
            gaps = [float(current[0]) - float(previous[2]) for previous, current in pairwise(ordered)]
...
class DocxAtsAnalyzer:
    def analyze(self, content: bytes) -> AtsDiagnosticData:
        ...
        score = max(0, 100 - min(45, table_count * 8) - (20 if complex_tables else 0))
        ...
        return AtsDiagnosticData(
            ats_friendly=score >= 75 and not complex_tables,
            score=score,
            confidence=0.9,
            ...
            metrics={"paragraphs": paragraph_count, "tables": table_count},
        )
